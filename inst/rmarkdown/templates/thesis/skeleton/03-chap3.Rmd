...
# This chunk ensures that the gcdown package is
# installed and loaded. This gcdown package includes
# the template files for the thesis and also two functions
# used for labeling and referencing
if (!require(remotes)) {
  if (params$`Install needed packages for {gcdown}`) {
...
if (!require(gcdown)) {
  if (params$`Install needed packages for {gcdown}`) {
    remotes::install_github("ismayc/gcdown")
  } else {
    stop(
      paste(
        "You need to run",
...
In the **R** chunk below, we will load in a picture stored as `gc_logo.png` in our main directory.  We then give it the caption of "graduate center logo", the label of "gclogo", and specify that this is a figure.  Make note of the different **R** chunk options that are given in the R Markdown file (not shown in the knitted document).

```{r gclogo, fig.cap="Graduate Center logo", out.width="0.2\\linewidth", fig.align="center"}
include_graphics(path = "figure/gc_logo.png")
```
...
Here is a reference to the Reed logo: Figure \@ref(fig:gclogo).  Note the use of the `fig:` code here.  By naming the **R** chunk that contains the figure, we can then reference that figure later as done in the first sentence here.  We can also specify the caption for the figure via the R chunk option `fig.cap`.
